...
public class AddApprovedRequest extends BaseServlet {
	
	private static final Logger log = Logger.getLogger(AddApprovedRequest.class);

	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		log.debug(AddApprovedRequest.class);
		...
		if (hasDomain == false) {
			try {
				...
				log.error("AddApprovedRequest * doPost - error 1: " + e.getMessage());
				e.printStackTrace();
			}
		} else {
...
//						}
					...
					als.addApprovedLeave(time, emailAddress, numOfDays, startDate,
							endDate, leaveType, supervisor, remark, region,"", 
							attachmentUrl, projectName);
					
					EmployeeLeaveDetailsService elds = new EmployeeLeaveDetailsService();
					
					Calendar calYear = Calendar.getInstance(Locale.getDefault());
					...
					if(calYear.get(Calendar.MONTH) > 2){
						...
						Double sickLeaveFP = Double.parseDouble(employeeD.getSickLeaveFP());
						...
						if(ConstantUtils.NO_PAY_LEAVE.equals(leaveType)){
							noPayLeave = Double.parseDouble(employeeD.getNoPayLeave()) +Double.parseDouble(numOfDays);
						}
						else if(ConstantUtils.SICK_LEAVE.equals(leaveType)){
							sickLeaveFP = Double.parseDouble(employeeD.getSickLeaveFP()) +Double.parseDouble(numOfDays);
						}
						else if(ConstantUtils.ANNUAL_LEAVE.equals(leaveType)){
							annualLeave = Double.parseDouble(employeeD.getAnnualLeave()) +Double.parseDouble(numOfDays);
						}
						else if(ConstantUtils.COMPANSSIONATE_LEAVE.equals(leaveType)){
							compassionateLeave = Double.parseDouble(employeeD.getCompassionateLeave()) +Double.parseDouble(numOfDays);
						}
						else if(ConstantUtils.BIRTHDAY_LEAVE.equals(leaveType)){
							birthdayLeave = Double.parseDouble(employeeD.getBirthdayLeave()) +Double.parseDouble(numOfDays);
						}
							...
							weddingLeave = Double.parseDouble(employeeD.getMarriageLeave()) +Double.parseDouble(numOfDays);
						}
						else if(ConstantUtils.OTHERS.equals(leaveType)){
							others = Double.parseDouble(employeeD.getOthers()) +Double.parseDouble(numOfDays);
						}
						
						if(ConstantUtils.ANNUAL_LEAVE.equals(leaveType)){
							balance = Double.parseDouble(employeeD.getBalance()) - Double.parseDouble(numOfDays);
						}
...
//						else if(ConstantUtils.SICK_LEAVE.equals(leaveType)){
//							sickLeave = Double.parseDouble(employeeD.getSickLeaveFP()) +Double.parseDouble(numOfDays);
//						}
						else if(ConstantUtils.ANNUAL_LEAVE.equals(leaveType)){
							annualLeave = Double.parseDouble(employeeD.getAnnualLeave()) +Double.parseDouble(numOfDays);
						}
...
//								balance.toString(), employeeD.getRegion());
					...
					errorMap.put("add.success", properties.getProperty("add.success"));
					req.setAttribute("errorMap", errorMap);
					getServletConfig().getServletContext().getRequestDispatcher("/feedback-map.jsp").forward(req, resp);
					return;
					...
					try {
						...
						log.error("AddApprovedRequest * doPost - error 3: " + e.getMessage());
						e.printStackTrace();
					}
				}
			} catch (Exception e1) {
...
//	    			}
	    			...
	    			if (existDirectory == false) {
	    				try {
	    					...
	    					log.error("AddApprovedRequest * doPost - error3: " + e.getMessage());
	    					e.printStackTrace();
	    				}
	    			}
	    		} catch (Exception e1) {
    		...
    		try{
    			...
    			log.error("AddApprovedRequest validate email error: " + e.getMessage());
    			e.printStackTrace();
    		}
