...
public class DeleteEmployee extends BaseServlet {
	
	private static final Logger log = Logger.getLogger(DeleteEmployee.class);

	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		log.debug(DeleteEmployee.class);
		String regionSelected = req.getParameter("cri_region");
		Vector delEmpVec = new Vector();
		// extracting data from the checkbox field
		String[] delEmployeeList = req.getParameterValues("delEmployeeList[]");
		
			if (delEmployeeList != null) {
				for (int i=0; i<delEmployeeList.length; i++) {
					log.error("DeleteEmployee - record to be deleted: " + delEmployeeList[i]);
					delEmpVec.add(delEmployeeList[i]);
				}
				for (int j=0; j<delEmpVec.size(); j++) {
					String name = delEmpVec.elementAt(j).toString();
					EmployeeService emps = new EmployeeService();
					for (MCEmployee emp : emps.getMCEmployees()) {
						if (emp.getEmailAddress().equalsIgnoreCase(name)) {
							emps.deleteMCEmployee(name);
						}				
					}
				}
				...
				try {
					...
					getServletConfig().getServletContext().getRequestDispatcher("/admin-delete-emp.jsp").forward(req, resp);
					return;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		log.debug(DeleteEmployee.class);
		String deptKeyStr = (String)request.getParameter("dept");
		...
		List<Employee> employeeList = EmployeeService.getInstance().getDepartmentEmployee(deptKeyStr);
		iTotalRecords = employeeList.size();
...
//							return c1.getResignedDate().compareTo(c2.getResignedDate()) * sortDirection;
				...
				try {
					...
					for(Employee employee : employeeList){
						...
						row.add(new JsonPrimitive("<a href=\"UpdateEmployee?edit="+
								employee.getEmailAddress() + "\"><i class=\"icon-edit\"></i></a>"));
						row.add(new JsonPrimitive("<input type=\"checkbox\" name=\"delEmployeeList\" value=\"" + employee.getEmailAddress() + "\"" + ">"));
