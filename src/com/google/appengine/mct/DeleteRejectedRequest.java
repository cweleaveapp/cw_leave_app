...
public class DeleteRejectedRequest extends BaseServlet {
	
	private static final Logger log = Logger.getLogger(DeleteRejectedRequest.class);
	

	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		String regionSelected = req.getParameter("cri_region");
		// extracting data from the checkbox field
		String[] delRejLQlist = req.getParameterValues("delRejLQlist[]");
		
		
			if (delRejLQlist != null) {
				for (int i=0; i<delRejLQlist.length; i++) {
					String id = delRejLQlist[i];
					RejectedLeaveService rls = new RejectedLeaveService();
					rls.deleteRejectedLeave(id);
				}
				
				try {
					req.setAttribute("cri_region", regionSelected);
					getServletConfig().getServletContext().getRequestDispatcher("/admin-delete-rejected.jsp").forward(req, resp);
					return;
				} catch (ServletException e) {
					log.error("DeleteRejectedRequest * doPost - error1: " + e.getMessage());
					e.printStackTrace();
				}
			} else {
				...
				try {
					...
					log.error("DeleteRejectedRequest * doPost - error2: " + e.getMessage());
					e.printStackTrace();
				}
			}
		}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		log.debug(DeleteRejectedRequest.class);
		...
		QueryResultList<Entity> results =  pq.asQueryResultList(FetchOptions.Builder.withDefaults());
			for(Entity result : results){
				...
				rejectLeave.setId(KeyFactory.keyToString(result.getKey()));
		...
		iTotalDisplayRecords = rejectList.size(); // number of history that match search criterion should be returned
		...
		try {
			...
			for(RejectedLeave rejectedLeave : rejectList){
				...
				row.add(new JsonPrimitive("<input type=\"checkbox\" name=\"delRejLQlist\" value=\"" + rejectedLeave.getId() + "\"" + ">"));
